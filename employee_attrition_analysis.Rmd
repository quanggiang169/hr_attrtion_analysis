...
ggplot(df_hratt, aes(x = num_companies_worked, fill = attrition)) + 
  geom_bar(position = "fill") + 
  scale_fill_manual(values = c("lightgray", "blue")) +  # Add a second color
...
p2 <- ggplot() +
  ...
  labs(title = "Attrition Probabilities per Years with Current Manager", 
       x = "Years with Current Manager", 
       fill = "Attrition") +
  scale_fill_manual(values = c("Employees who left" = "lightgray", "Employees who stayed" = "blue")) +
  theme_minimal(base_size = 15) +
  theme(plot.title = element_text(size = 20, hjust = 0.5),
        axis.title.x = element_text(size = 16),
        legend.title = element_text(size = 15),
...
```{r, echo=FALSE}
# Function to calculate Mutual Information for all combinations of categorical variables
calculate_mutual_information <- function(data) {
  
  # Create a contingency matrix for the remaining variables
  variables <- colnames(data)
  mi_matrix <- matrix(NA, nrow = length(variables), ncol = length(variables))
  rownames(mi_matrix) <- variables
  colnames(mi_matrix) <- variables
  
  # Calculate MI for each pair of categorical variables
  for (i in 1:length(variables)) {
    for (j in i:length(variables)) {
      # Compute Mutual Information for the pair of variables
      mi_value <- mutinformation(as.factor(data[[variables[i]]]), as.factor(data[[variables[j]]]))
  ...
  mi_df <- as.data.frame(as.table(mi_matrix))
...
# Example dataset
cat_attributes <- data.frame(
  job_role = sample(c('Manager', 'Developer', 'Analyst'), 100, replace = TRUE),
  department = sample(c('HR', 'Engineering', 'Sales'), 100, replace = TRUE),
  business_travel = sample(c('Yes', 'No'), 100, replace = TRUE)
)

# Calculate MI for your dataset
mi_result <- calculate_mutual_information(cat_attributes)

# Print the MI results
print(mi_result)

# Plotting the heatmap
ggplot(mi_result, aes(Var1, Var2, fill = Freq)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "blue") +  # Gradient from white to blue
  geom_text(aes(label = round(Freq, 2)), color = "black", size = 6) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Mutual Information Heatmap", x = "Variables", y = "Variables") +
